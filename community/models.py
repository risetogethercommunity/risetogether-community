...
class DSAActivity(models.Model):
    ...
    user = models.ForeignKey(User, related_name="dsa_activities", on_delete=models.CASCADE)
...
class Skill(models.Model):
    ...
    icon_class = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="CSS class e.g. fa-brands fa-html5"
    )
    icon_image = models.ImageField(
        upload_to="skill_icons/",
        blank=True,
        null=True,
        help_text="Upload PNG/SVG if not using an icon class"
    )
...
class Activity(models.Model):
    ...
    date = models.DateField(blank=True, null=True)  # only used if occurrence = once
    created_at = models.DateTimeField(auto_now_add=True)
...
class Project(models.Model):
    ...
    leader = models.ForeignKey(User, related_name="led_projects", on_delete=models.SET_NULL, null=True, blank=True)
    members = models.ManyToManyField(User, related_name="projects", blank=True)
    special_highlight = models.CharField(
        max_length=255,
        blank=True,
        null=True,
        help_text="e.g. Featured at XYZ, Award Winner"
    )
...
class Post(models.Model):
    ...
    hashtags = models.CharField(
        ...
        help_text="Comma-separated hashtags e.g. #dsa,#coding,#python"
    )
